...
test( 'test single -t,--target', function ( t ) {
  ...
  prepareStage( function ( err ) {
    ...
    const spawn = cp.spawn(
      ...
      -P ${ 40666 }
      -p ${ publicPath }
      -t ${ path.join( publicPath, 'index.html' ) }
      `.trim().split( /\s+/ )
    )
    spawns.push( spawn )
    ...
    function cpyHTML ( done ) {
      // rewrite file to trigger file watcher and target-build event
      var text = fs.readFileSync( path.join( __dirname, 'stage', 'index.html' ), 'utf8' )
      fs.writeFileSync( path.join( __dirname, 'stage', 'index.html' ), text, 'utf8' )
    ...
    function end () {
      t.ok(
        stripAnsi( log ).indexOf( '(file change detected) \nbroadcasting: test/stage/index.html' ) >= 0,
        'index.html target-build OK!'
      )
