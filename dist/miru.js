...
module.exports={
  "base64": ""
}
...
var argv = require('subarg')(process.argv.slice(2), {
  ...
  boolean: ['noflicker', // disable style reload flicker ( not recommended )
  'nolimit', // nolimit on col size terminal output
  'version', 'verbose', 'help', 'no-wooster', 'no-miru-connect', 'no-clear', // disable terminal clearing
  'development'],
...
var _watchers = [];
if (argv['watch']) {
  _watchers = argv['watch'];

  if (!(_watchers instanceof Array)) {
    _watchers = [_watchers];
  }
...
try {
  ...
  fs.readFileSync(miruConnectDestination).equals(injectMiruConnect());

  log('up to date miru-connect.js found at: ' + miruConnectDestination);

  if (argv['development']) {
    var w = miteru.watch(path.join(__dirname, '../../dist/miru-connect.js'), function (evt, filepath) {
      switch (evt) {
        case 'add':
        case 'change':
          writeMiruConnect();
...
function injectMiruConnect() {
  // load the bundled miru-connect.js from the dist directory
  var text = fs.readFileSync(path.join(__dirname, '../../dist/miru-connect.js'), 'utf8');

  // inject it with options and variables
  text = '\n    ;(function () {\n      window.__miru = {\n        verbose: true,\n        styleFlicker: ' + !argv['noflicker'] + ',\n        targets: ' + JSON.stringify(Object.keys(_targets)) + '\n      }\n    })();\n    ' + text + '\n  ';
...
io.on('connect', function (socket) {
  ...
  Object.keys(_targets).forEach(function (target) {
...
function parseClientID(userAgent) {
  ...
  id = id.split(/\s+/).filter(function (word) {
    var w = word.toLowerCase().trim();
    return w !== 'other' && w !== '0.0.0' && w !== 'undefined' && w;
  }).join(' ').trim();
...
server.listen(PORT, ADDRESS, function () {
  ...
  console.log('server listening at *:' + addr.port + ' ( ' + addr.family + ' ' + addr.address + ' )');
...
function launchRecoveryWatcher(w) {
  ...
  if ( // style files
  target.indexOf('.css') > 0 || target.indexOf('.styl') > 0 || target.indexOf('.sass') > 0 || target.indexOf('.scss') > 0 || target.indexOf('.less') > 0) {
  ...
  target.indexOf('.js') > 0 || target.indexOf('.jsx') > 0) {
...
function handleError(target, text, ranking) {
  ...
  if (prevError) {
    ...
    if (delta < errors.THROTTLE && (prevError.raw === error.raw || prevError.text === error.text || error.context && prevError.context === error.context)) {
  ...
  if (!shouldSkip) {
    ...
    errors.timeout = setTimeout(function () {
      ...
      var t = _targets[path.resolve(target)];
      if (t) {
        t.output = output;
      }
...
function getIterationErrorBox() {
  ...
  _iterationBoxErrorCounter = (_iterationBoxErrorCounter + 1) % _iterationBoxErrorLimit;
      ...
      fs.readFile(filepath, 'utf8', function (err, text) {
        ...
        if (hasErrors === false) {
          ...
          executions.forEach(function (command) {
            ...
            command = command.split('$evt').join(evt).split('$file').join(filepath);
          ...
          _targets[target].error = undefined;
          _targets[target].output = undefined;
          ...
          var target = path.resolve(filepath);
          if (!_targets[target].error) {
            log('target error attached');
            _targets[target].error = error;
          }
        }
      });
  } // switch ( evt )
...
var commands = {
  ...
  'pesticide': function pesticide(args) {
    /*
  ...
  'watchers': function watchers(args) {
...
process.stdin.on('data', function (chunk) {
  ...
  var keys = Object.keys(commands).filter(function (command) {
  ...
  if (typeof fn === 'function') {
    _lastCommand = function _lastCommand() {
      fn(args);
    };
    fn(args);
  } else {
...
}).call(this,"/Users/mollie/code/miru/src/server")
},{"./favicon.json":1,"assert":undefined,"child_process":undefined,"cli-color":undefined,"cors":undefined,"express":undefined,"fs":undefined,"http":undefined,"miteru":undefined,"os":undefined,"path":undefined,"socket.io":undefined,"string-width":undefined,"subarg":undefined,"ua-parser-js":undefined,"wooster":undefined}]},{},[2]);
